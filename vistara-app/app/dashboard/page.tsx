...
export default function Verification() {
    ...
    <section className="mx-auto max-w-4xl px-6 py-12">
        <Link href="/host/property/new" className="text-sm font-medium text-[#03045e] hover:text-[#023e8a]">
            ← Back to Property
        </Link>
<div className="mt-8">
    ...
    <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-500">
        Verification helps Vistara build trust between hosts and travelers. Complete the required checks before your property goes live.
    </p>
    </div>
    <div className="mt-8 flex items-center gap-3">
        ...
        <div className="mt-10 space-y-4" >
            {verificationItems.map((item, index) => (
                ...
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#03045e] text-white">
                0{index + 1}
                </div>
                <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-lg font-semibold text-[#03045e]">{item.title}</h2>
                <span className="rounded-full bg-[#e0e7ff] px-3 py-1 text-xs font-medium text-[#03045e]">
                    {item.status}
                </span>
                </div>
                <p className=" mt-2 leading-6 text-sm text-gray-500">

                    {item.description}
                </p>
                </div>
                <button  type="button" className="mt-4 rounded-xl bg-[#03045e] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#023e8a] sm:mt-0">
                    Upload
                </button>
                </div>
                </div>
